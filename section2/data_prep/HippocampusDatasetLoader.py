...
def LoadHippocampusData(root_dir, y_shape, z_shape):
    ...
    for f in images:
        ...
        # <YOUR CODE GOES HERE>
        image=image.astype(np.single)/0xff#np.max(image)#(image-np.mean(image))/np.std(image)
        ...
        # ANSWER: During training int labels are expected, representing distinct classes

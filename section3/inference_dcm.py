...
def create_report(inference, header, orig_vol, pred_vol):
    ...
    header_font = ImageFont.truetype("assets/arial.ttf", size=40) #load_default()# Roboto-Regular.ttf
    main_font = ImageFont.truetype("assets/arial.ttf", size=20)
...
#         model=None)
    ...
    pred_label = inference_agent.single_volume_inference_unpadded(volume) #np.array(volume)
    ...
    os_command("storescu localhost 4242 -v -aec HIPPOAI +r +sd '/home/workspace/out/report.dcm'")#{report_save_path}")
